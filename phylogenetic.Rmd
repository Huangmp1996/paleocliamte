...
all_taxon <- c('amphibian','reptile','ave','mammal')
Chinese_species_distribution_grid_selected <- get_species_distribution_of_taxon(all_taxon)
Chinese_species_distribution_grid_selected$className <- str_extract(Chinese_species_distribution_grid_selected$taxa,
                                                                    '^[A-Z]+(?=_)')
phy <- read.tree("./phylogenetic_tree/02.tree/tree2/tree2.treefile.dated.tree_with_outgroup")
...
ggplot()+geom_sf(data = south_ocean,aes(geometry = geometry),colour='grey60',fill='grey90',size=0.2)+
    geom_sf(data = china_map,aes(geometry = geometry),colour='black',fill='grey90',size=0.2)+
...
```{r}
region_df <- phylore$region.df %>% select(grids,cluster,COLOURS) %>% within({cluster <- as.factor(cluster)})
```
...
ggplot()+geom_sf(data = south_ocean,aes(geometry = geometry),colour='grey60',fill='grey90',size=0.2)+
    ...
    scale_fill_manual(values =c("#0072B2", "#AC84A8","#CD9CC8",  "#8034FB","#F69EFF", "#FFCAA0","#BC1BFD" , "#A354FF",
                                "#B3CAD6","#E4914F","#FE6544","#419BBC","#D55E00","#CC0033","#690000"))+ # fill 
...
plot(realm$geometry)
# st_write(realm,'phyloregions.shp')
```

```{r}
region_phylo <- as.phylo(hclust(phylore$region.dist))
gpinfo <- data.frame(region=c(1:15),color = c("#0072B2", "#AC84A8","#CD9CC8",  "#8034FB","#F69EFF", "#FFCAA0","#BC1BFD" , "#A354FF",
                                "#B3CAD6","#E4914F","#FE6544","#419BBC","#D55E00","#CC0033","#690000"))
ggtree(region_phylo) %<+% gpinfo+ geom_tiplab(size=10,aes(col = color))+ theme_tree2()
...
stopCluster(cl)
saveRDS(beta_ratio_Baselga,'beta_ratio_Baselga.rds')
```

```{r}
beta_ratio_Baselga$i <- as.character(beta_ratio_Baselga$i)
grid$id <- as.character(grid$id)
beta_ratio_Baselga_map <- left_join(beta_ratio_Baselga,grid,by = c('i'='id'))
options(repr.plot.width=14, repr.plot.height=12)
ggplot()+geom_sf(data = south_ocean,aes(geometry = geometry),colour='grey60',fill='grey90',size=0.2)+
    geom_sf(data = china_map,aes(geometry = geometry),colour='grey60',fill='grey90',size=0.2)+
    geom_sf(data = beta_ratio_Baselga_map,aes(fill = ratio,geometry = geometry),colour='white',alpha=0.8,size=0)+
    scale_fill_distiller(palette = "Spectral")+ # fill 
    geom_sf(data = phyloregions_line,aes(geometry = geometry,col=divider,size = divider))+
    scale_color_manual(values = c('grey60','red','#F75404'),guide = "none")+
    scale_size_manual(values = c(0.5,0.8,0.6),guide = "none")+
...
```{r}
library(BAT)
beta_ratio_cal <- function(i,submat, subphy){
    submat <- as.matrix(submat)
    no. <-  c((i-103):(i-101),i %>% -1,i,i+1,(i+101):(i+103)) %>% as.character()
    subcomm <- subset(submat,row.names(submat) %in% no.)
    subcomm <- subcomm[,colSums(subcomm) != 0]
    phy_tmp <- ape::keep.tip(subphy,colnames(subcomm))
    beta <- BAT::beta(subcomm,phy_tmp,abund = F, raref = 0,runs = 0)
    sor_mean <- (beta$Btotal %>% as.matrix() %>% as.data.frame() %>% pull(as.character(i)) %>% sum())/8
    sne_mean <- (beta$Brich %>% as.matrix() %>% as.data.frame() %>% pull(as.character(i)) %>% sum())/8
    ratio <- sne_mean/sor_mean
    result <- data.frame(i,ratio)
    return(result)
}
...
cl <- makeCluster(30)      
registerDoParallel(cl)       #进行进程注册
beta_ratio_Carvalho <- foreach(
    i = as.numeric(unique(row.names(submat))),          #输入等待请求的参数
    .export = c('submat', 'subphy'),
    .combine = rbind,  #返回结果的整合
    .packages = c("dplyr","magrittr","BAT","ape") #多个进程共享的系统环境
) %dopar% beta_ratio_cal(i,submat,subphy)
stopCluster(cl)
saveRDS(beta_ratio_Carvalho,'beta_ratio_Carvalho.rds')
```

```{r}
# beta_ratio_Carvalho <- readRDS('beta_ratio_Carvalho.rds')
beta_ratio_Carvalho$i <- as.character(beta_ratio_Carvalho$i)
grid$id <- as.character(grid$id)
beta_ratio_Carvalho_map <- left_join(beta_ratio_Carvalho,grid,by = c('i'='id'))
options(repr.plot.width=14, repr.plot.height=12)
ggplot()+geom_sf(data = south_ocean,aes(geometry = geometry),colour='grey60',fill='grey90',size=0.2)+
    geom_sf(data = china_map,aes(geometry = geometry),colour='grey60',fill='grey90',size=0.2)+
    geom_sf(data = beta_ratio_Carvalho_map,aes(fill = ratio,geometry = geometry),colour='white',alpha=0.8,size=0)+
    scale_fill_distiller(palette = "Spectral")+ # fill 
    geom_sf(data = phyloregions_line,aes(geometry = geometry,col=divider,size = divider))+
    scale_color_manual(values = c('grey60','red','#F75404'),guide = "none")+
    scale_size_manual(values = c(0.5,0.8,0.6),guide = "none")+
    ...
    theme(panel.grid.major=element_blank(),
         ...
         legend.title=element_text(size=13),
         legend.text=element_text(size=10))
ggsave('beta_nestedness_ratio_Carvalho.tiff',width = 7,height = 7,units = 'in',dpi = 300)
```

#### taxonomic groups

```{r}
get_species_of taxon <- function(){}
```

```{r}
# beta_ratio_cal_taxon <- function(i,submat,subphy,taxon){
#     filter()
#     submat <- as.matrix(submat)
#     no. <-  c((i-103):(i-101),i %>% -1,i,i+1,(i+101):(i+103)) %>% as.character()
#     subcomm <- subset(submat,row.names(submat) %in% no.)
...
df <- left_join(beta_ratio_Carvalho,env,by = c('i'='id')) %>% select(i, x, y, prec, tavg, LGM_tavg_velocity,dem_value,ratio) %>% na.omit() 
...
```{r}
ggplot(data = df,aes(x=LGM_tavg_velocity,y=ratio))+
geom_point()
```
