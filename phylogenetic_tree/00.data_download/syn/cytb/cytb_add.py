    ...
    for j in SeqRecord.features:
        if j.type == 'CDS':
            if 'gene' in j.qualifiers.keys():
                if j.qualifiers['gene'][0].upper() in ['CYTB','CYT B','CYTOCHROME B','CB','CYTCHROME B']:
                ...
                if j.qualifiers['product'][0].upper() in ['CYTB','CYT B','CYTOCHROME B','CB','CYTCHROME B']:
                    if len(j.location) == len(str(SeqRecord.seq)[j.location.start:j.location.end]):
                        m1 = j.location.start
                        n1 = j.location.end
                        if len(str(SeqRecord.seq)[m1:n1]) < 300:continue
...
def cytb_add(taxa1,taxa2):
    ...
    content = '''(cytb[All Fields] OR "cyt b"[All Fields] OR "cytochrome b"[All Fields]) AND ("{}"[Organism]) NOT PREDICTED[title]'''.format(species_name)
    ...
    out_add_file_name = 'cytb_add/' + taxa1 + '.txt'
    open(out_add_file_name,'w').write(text)
    out_dl_file_name = 'cytb_download/' + taxa1 + '.fasta'
    open(out_dl_file_name,'w').write(download_seq)
    
if __name__ == '__main__':
    cytb_add(os.sys.argv[1],os.sys.argv[2])                     
