    ...
    for j in SeqRecord.features:
        if j.type == 'CDS':
            if 'gene' in j.qualifiers.keys():
                if j.qualifiers['gene'][0].upper() in [
                    'CO1','COI','COXI','COX1',
                    'CYTOCHROME OXIDASE SUBUNIT 1','CYTOCHROME OXIDASE SUBUNIT I',
                    'CYTOCHROME C OXIDASE SUBUNIT 1','CYTOCHROME C OXIDASE SUBUNIT I'
                    ]:
                ...
                if j.qualifiers['product'][0].upper() in [
                    'CO1','COI','COXI','COX1',
                    'CYTOCHROME OXIDASE SUBUNIT 1','CYTOCHROME OXIDASE SUBUNIT I',
                    'CYTOCHROME C OXIDASE SUBUNIT 1','CYTOCHROME C OXIDASE SUBUNIT I'
                    ]:
...
def coI_add(taxa1,taxa2):
    ...
    content = '''(CO1[All Fields] OR "COX1"[All Fields] OR "COI"[All Fields] OR "COXI"[All Fields] OR "cytochrome oxidase subunit 1"[All Fields]) AND ("{}"[Organism]) NOT PREDICTED[title]'''.format(species_name)
    ...
    out_add_file_name = 'co1_add/' + taxa1 + '.txt'
    open(out_add_file_name,'w').write(text)
    out_dl_file_name = 'co1_download/' + taxa1 + '.fasta'
    open(out_dl_file_name,'w').write(download_seq)
    
if __name__ == '__main__':
    coI_add(os.sys.argv[1],os.sys.argv[2])                     
