...
def seq_check(SeqRecord,taxa):
    ...
    for j in SeqRecord.features:
        if j.type == 'rRNA':
            if j.qualifiers['product'][0].upper() in ['16S RIBOSOMAL RNA', 'L-RRNA', '16S RRNA','LARGE SUBUNIT RIBOSOMAL RNA','16S LARGE SUBUNIT RIBOSOMAL RNA']:
...
def RNA_add(taxa):
    ...
    content = '''("16S"[All Fields] OR "large subunit ribosomal RNA"[All Fields]) AND ("{}"[Organism]) NOT PREDICTED[title]'''.format(species_name)
    ...
    if 'IdList' in records.keys(): # .read:解析结果 
        ...
        if len(ids) != 0: 
            for id in ids: 
                ...
                text += taxa + '\t' + str(SeqRecord.id) + '\t' + str(len(SeqRecord.seq)) + '\t' + SeqRecord.description + '\n'
                download_seq += seq_check(SeqRecord,taxa)
    out_add_file_name = '16S_RNA_add/' + taxa + '.txt'
    open(out_add_file_name,'w').write(text)
    out_dl_file_name = '16S_RNA_download/' + taxa + '.fasta'
    open(out_dl_file_name,'w').write(download_seq)

if __name__ == '__main__':
    RNA_add(os.sys.argv[1])                     
